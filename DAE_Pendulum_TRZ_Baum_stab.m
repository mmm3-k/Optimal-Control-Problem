...
%Initialize guess
X0 = repmat([L;0;0;0],1, N+1);
Z0 = zeros(nz,N+1);
U0 = zeros(nu, N+1);
w0 = [X0(:); Z0(:); U0(:)];

lbX = -inf * ones(nx, N+1); ubX = inf * ones(nx, N+1);
lbZ = -inf * ones(nz, N+1); ubZ = inf * ones(nz, N+1);
lbU = -inf * ones(nu, N+1); ubU = inf * ones(nu, N+1);
...
for i = 1:N
	idx_k = i;
	idx_kpl = i+1;
	L_running = L_running + (dt/2)*(U(:,idx_k)^2 + U(:,idx_kpl)^2);
end
